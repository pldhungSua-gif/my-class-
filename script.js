...
const BACKEND_API_URL = "https://my-class-backend.onrender.com/api/announcements";

// Mảng chứa dữ liệu nhận từ Backend
...
function home() {
  ...
  <section class="stats"><div><b>${members.length}</b><span>Thành viên</span></div><div><b>${Math.round(
    members.reduce((s, m) => s + m[2], 0) / members.length
  )}</b><span>Điểm TB</span></div><div><b>${events.length}</b><span>Sự kiện sắp tới</span></div><div><b>∞</b><span>Năng lượng</span></div></section>
  <section class="grid-2">
    <div class="card"><div class="card-title"><h2>🏆 Top thi đua</h2><a href="#ranking">Xem tất cả →</a></div>${top
      .map((m, i) => rankRow(m, i + 1))
      .join("")}</div>
    <div class="card"><div class="card-title"><h2>📢 Mới nhất</h2><a href="#announcements">Tất cả →</a></div>${announcements
      .slice(0, 3)
      .map(
        (a) =>
          `<article class="mini-news"><span>${a.tag}</span><b>${a.title}</b><small>${a.date}</small></article>`
      )
      .join("")}</div>
  </section>`;
}
...
function rankRow(m, i) {
  return `<div class="rank-row"><strong class="rank-no">${i}</strong><div class="avatar">${initials(
    m[0]
  )}</div><div class="grow"><b>${m[0]}</b><small>${m[3]}</small></div><strong class="score">${
    m[2]
  }đ</strong></div>`;
}
...
function membersPage() {
  return layout(
    "Thành viên lớp",
    "Gặp gỡ 37 thành viên tuyệt vời của lớp 10 Toán 1",
    `
    <div class="members-filter-bar">
      ...
      <div class="filter-pills">
        <button class="pill-btn ${
          currentMemberRoleFilter === "all" ? "active" : ""
        }" onclick="filterMemberRole('all', this)">Tất cả</button>
        <button class="pill-btn ${
          currentMemberRoleFilter === "bcs" ? "active" : ""
        }" onclick="filterMemberRole('bcs', this)">Ban cán sự</button>
        <button class="pill-btn ${
          currentMemberRoleFilter === "member" ? "active" : ""
        }" onclick="filterMemberRole('member', this)">Thành viên</button>
      </div>
    </div>
...
function handleSearchMembers(keyword) {
  ...
  if (container) container.innerHTML = renderMembersList();
}

function filterMemberRole(role, btn) {
  currentMemberRoleFilter = role;
  document
    .querySelectorAll(".members-filter-bar .pill-btn")
...
function rankingPage() {
  ...
  return layout(
    "Bảng xếp hạng",
    "Theo dõi điểm thi đua và thứ hạng các thành viên",
    `
    ...
    <div id="pointRulesSection" style="display: ${
      showPointRules ? "grid" : "none"
    }; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 32px;">
    ...
    <div style="display: grid; grid-template-columns: 1fr 1.1fr 1fr; gap: 20px; align-items: flex-end; margin-bottom: 36px; padding: 0 20px;">
      <div style="background: #ffffff; border-radius: 20px; padding: 28px 16px; text-align: center; border: 1px solid #f0ebff; box-shadow: 0 4px 16px rgba(0,0,0,0.02);">
        ...
        <div style="font-weight: 800; font-size: 15px; color: #17182d; margin-bottom: 4px;">${
          top2[0]
        }</div>
        <div style="font-size: 13px; font-weight: 700; color: #624cff;">${
          top2[2]
        } điểm</div>
      </div>
      ...
      <div style="background: #f6f5ff; border-radius: 20px; padding: 36px 16px; text-align: center; border: 2px solid #e2d9ff; box-shadow: 0 8px 24px rgba(98, 76, 255, 0.08); transform: translateY(-10px);">
        ...
        <div style="font-weight: 800; font-size: 16px; color: #17182d; margin-bottom: 4px;">${
          top1[0]
        }</div>
        <div style="font-size: 13.5px; font-weight: 800; color: #624cff;">${
          top1[2]
        } điểm</div>
      </div>
        ...
        <tbody>
          ${sorted
            ...
            <tr style="border-bottom: 1px solid #f4f3ff; font-size: 13.5px; transition: background 0.2s;" onmouseover="this.style.background='#fbfaff'" onmouseout="this.style.background='transparent'">
              <td style="padding: 14px 20px; font-weight: 700; color: #4a4e69;">${
                idx + 1
              }</td>
              <td style="padding: 14px 20px;">
                <a href="#member-${members.indexOf(
                  m
              ...
              <td style="padding: 14px 20px; text-align: right; font-weight: 800; color: #17182d;">${
                m[2]
              }</td>
            </tr>
          `
...
function togglePointRules() {
  ...
  if (section) section.style.display = showPointRules ? "grid" : "none";
}

// ----------------------------------------------------
// TRANG THÔNG BÁO & SỰ KIỆN (KẾT NỐI MONGODB BACKEND)
// ----------------------------------------------------
...
function announcementsPage() {
  ...
  return layout(
    "Thông báo & Sự kiện",
    "Cập nhật những thông tin và sự kiện mới nhất của lớp",
    `
      ...
      <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
        <div style="display: flex; gap: 6px; background: #eae7ff; padding: 4px; border-radius: 20px;">
          <button 
            ...
            style="padding: 8px 18px; border-radius: 16px; border: none; font-weight: 700; font-size: 13.5px; cursor: pointer; transition: all 0.2s; ${
              currentTab === "announcements"
                ? "background: #624cff; color: #fff; box-shadow: 0 2px 8px rgba(98, 76, 255, 0.25);"
                : "background: transparent; color: #624cff;"
            }"
          >
            🔔 Thông báo
          </button>
        ...
        >
          <span style="font-size: 15px;">+</span> ${
            currentTab === "announcements" ? "Tạo thông báo" : "Tạo sự kiện"
          }
        </button>
      </div>
...
// Gọi API lấy dữ liệu từ MongoDB Atlas
async function loadAnnouncementsFromBackend() {
  ...
  container.innerHTML = `<div style="text-align: center; padding: 30px; color: #6b7280;">⏳ Đang tải dữ liệu từ máy chủ...</div>`;

  try {
    const response = await fetch(BACKEND_API_URL);
    if (!response.ok) throw new Error("Lỗi kết nối");

    backendAnnouncements = await response.json();
...
function switchMainTab(tab) {
  ...
  if (tab === "announcements") {
    btnA.style.cssText =
      "padding: 8px 18px; border-radius: 16px; border: none; font-weight: 700; font-size: 13.5px; cursor: pointer; transition: all 0.2s; background: #624cff; color: #fff; box-shadow: 0 2px 8px rgba(98, 76, 255, 0.25);";
    btnE.style.cssText =
      "padding: 8px 18px; border-radius: 16px; border: none; font-weight: 700; font-size: 13.5px; cursor: pointer; transition: all 0.2s; background: transparent; color: #624cff;";
    if (btnCreate)
      btnCreate.innerHTML =
        '<span style="font-size: 15px;">+</span> Tạo thông báo';
    ...
    if (btnCreate)
      btnCreate.innerHTML =
        '<span style="font-size: 15px;">+</span> Tạo sự kiện';
...
// Gửi thông báo lên CSDL MongoDB
async function submitNotice(e) {
  ...
  const newNotice = { title, content, tag, date: dateStr };
  ...
  try {
    ...
    if (res.ok) {
      ...
      showToast("Không thể lưu thông báo!");
    }
  } catch (error) {
...
function renderMainTabContent() {
            ...
            <div>
              <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
                <div style="display: flex; align-items: center; gap: 10px;">
                  ...
                  <div>
                    <h4 style="font-weight: 800; font-size: 15px; color: #17182d; margin: 0;">${
                      ev.title
                    }</h4>
                    <small style="color: #624cff; font-weight: 700;">${
                      ev.date
                    }</small>
                  </div>
                </div>
              </div>
              <p style="font-size: 13px; color: #555770; margin: 6px 0 0 0; line-height: 1.5;">${
                ev.desc
              }</p>
            </div>
            <div>
  ...
  // Danh sách Thông báo lấy từ MongoDB
  const filteredNotices = backendAnnouncements.filter((a) => {
    if (currentNoticeFilter === "all") return true;
    return a.tag === currentNoticeFilter;
  });

  if (!filteredNotices || filteredNotices.length === 0) {
    return `<p style="text-align:center; color:#888; padding: 40px 0;">📭 Chưa có thông báo nào trong danh mục này.</p>`;
  }
    ...
    <div style="display: flex; flex-direction: column; gap: 16px;">
      ${filteredNotices
        ...
        <div style="background: #ffffff; border-radius: 16px; padding: 20px 24px; border: 1px solid #e5e0ff; box-shadow: 0 4px 16px rgba(0,0,0,0.02); display: flex; justify-content: space-between; align-items: flex-start; gap: 16px;">
          <div style="flex: 1;">
            <h4 style="font-weight: 800; font-size: 15px; color: #17182d; margin: 0 0 8px 0;">${
              a.title
            }</h4>
            <p style="font-size: 13.5px; color: #555770; margin: 0 0 12px 0; line-height: 1.5;">${
              a.content
            }</p>
            <small style="color: #624cff; font-weight: 700;">🕒 ${
              a.date || "Mới đây"
...
function setNoticeFilter(filter) {
  ...
  const filterContainer = document.getElementById("noticeFilterContainer");
  if (filterContainer) filterContainer.innerHTML = renderNoticeFilters();

  const contentContainer = document.getElementById("mainTabContentContainer");
  if (contentContainer) contentContainer.innerHTML = renderMainTabContent();
}
...
// ----------------------------------------------------
// CÁC TRANG PHỤ Bổ sung & ĐIỀU HƯỚNG
// ----------------------------------------------------
function eventsPage() {
  return layout(
    "Sự kiện",
    "Đừng để lịch lớp chạy nhanh hơn bạn 😎",
    "<div class='event-grid'>" +
      events
        .map(
          (e) =>
            `<article class='event card'><div class='date-box'><b>${new Date(
              e.date + "T00:00:00"
            ).getDate()}</b><small>${new Date(
              e.date + "T00:00:00"
            )
              .toLocaleDateString("vi-VN", { month: "short" })
              .toUpperCase()}</small></div><div><span class='tag'>SẮP TỚI</span><h2>${
              e.title
            }</h2><p>🕐 ${e.time} · 📍 ${e.place}</p></div></article>`
        )
        .join("") +
      "</div>"
  );
...
function schedulePage() {
  return layout(
    "Thời khóa biểu",
    "Lịch học mẫu của lớp 10 Toán 1.",
    "<div class='card schedule-wrap'><table><thead><tr><th>Ngày</th><th>Tiết 1</th><th>Tiết 2</th><th>Tiết 3</th><th>Tiết 4</th><th>Tiết 5</th></tr></thead><tbody>" +
      schedule
        .map(
          (r) =>
            `<tr>${r
              .map(
                (x, i) => `<td class="${i === 0 ? "day" : ""}">${x}</td>`
              )
              .join("")}</tr>`
        )
        .join("") +
